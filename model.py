...
class GPTConfig:
    ...
    # extra in-place multiplier on every c_proj.weight (the residual-branch output projections),
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, ref_logits=None):
        # ref_logits: logits of a frozen copy of the model at init, for alpha-centering. Only
        # used when targets are given; the logits become logits - ref_logits
        device = idx.device
        b, t = idx.size()
        ...
        if targets is not None:
            ...
            if self.config.alpha != 1.0:
                logits = logits * self.config.alpha
